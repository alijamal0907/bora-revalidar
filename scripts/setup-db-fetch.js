...
async function checkTables() {
  ...
  for (const table of tables) {
    try {
      ...
      console.log(`Tabela ${table}: ${response.ok ? 'EXISTE' : 'NAO EXISTE'}`)
    } catch {
      results[table] = false
      console.log(`Tabela ${table}: NAO EXISTE (erro)`)
    }
  }
...
async function main() {
  ...
  console.log('SERVICE_ROLE_KEY:', SERVICE_ROLE_KEY ? 'OK (oculto)' : 'FALTANDO')
  
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    console.log('ERRO: Variaveis de ambiente nao configuradas')
    return
  }
  ...
  if (allExist) {
    ...
    console.log('Setup de gamificacao ja esta completo.')
  } else {
    console.log('\n*** TABELAS FALTANDO ***')
    console.log('Execute o SQL manualmente no Supabase Dashboard:')
    console.log('1. Va para https://supabase.com/dashboard')
    console.log('2. Selecione seu projeto')
    console.log('3. Va para SQL Editor')
    console.log('4. Cole o conteudo de scripts/create-gamification-tables.sql')
  }
  
  console.log('\n=== STATUS FINAL ===')
  for (const [table, exists] of Object.entries(tableStatus)) {
    console.log(`  ${exists ? '[OK]' : '[  ]'} ${table}`)
  }
}
